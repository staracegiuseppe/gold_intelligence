...
MIN_CONF_FOR_ACTIVE = 40  # confidence minima per BUY/SELL (40% = 3+ layer concordi)
...
def composite_signal(
    ...
    f_fair_value  = fundamental_data.get("fair_value",  {})         if fundamental_data else {}
    f_health      = fundamental_data.get("health_score",{})         if fundamental_data else {}
    ...
    # ── Layer agreement — calcolato SOLO sui layer con dati reali ────────────
    # Layer con score=0 senza dati (no FMP, no FRED) non contano come discordanti
    direction = 1 if composite_int > 0 else -1
    all_scores = [
        tech_norm, macro_score, regime_score, sector_score, f_score, i_score
    ]
    active_scores = [s for s in all_scores if s != 0]  # solo layer con dati reali
    if not active_scores:
        active_scores = [tech_norm]  # fallback: almeno il tecnico
    agreeing = sum(1 for s in active_scores if s * direction > 0)
    agreement_pct = agreeing / len(active_scores)
    ...
    # Regime e settore
    if regime_detail.get("regime"):
        composite_reasons.append(f"Regime di mercato: {regime_detail['regime']} (VIX={regime_detail.get('vix','?')})")
    if sector_detail.get("narrative"):
        composite_reasons.append(sector_detail["narrative"])
    # Istituzionale
    if i_detail.get("narrative"):
        composite_reasons.append(i_detail["narrative"])
    ...
    result = {
        **technical_signal,
        ...
        "fair_value":         f_fair_value,
        "health_score":       f_health,
    ...
    # De-duplica le reasons
    seen = set()
    deduped = []
    for r in composite_reasons + technical_signal.get("reasons", []):
        if r not in seen:
            seen.add(r); deduped.append(r)
    result["reasons"] = deduped[:7]
