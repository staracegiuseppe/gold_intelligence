...
def run_scan():
    ...
    if FUND_ENABLED and FMP_KEY:
        ...
        try:
            # Passa i prezzi correnti per calcolare Fair Value upside
            prices_map = {sym: ind_data.get("last_price") 
                          for sym, ind_data in tech.items() if ind_data}
            fund_db = fetch_all_fundamentals(ASSETS, FMP_KEY, prices_map)
